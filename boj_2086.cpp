...
long long CalcFibo(long long lN)
{
	...
	while (lN)
	{
		if (lN & 1)
		{
			...
			for (int i = 0; i < 2; ++i) for (int j = 0; j < 2; ++j) matFibo[i][j] = (matTemp[i][j]) % 1000000000;
		}
		vector<vector<long long>> matTemp(2, vector<long long>(2, 0));
		for (int i = 0; i < 2; ++i) for (int j = 0; j < 2; ++j) for (int k = 0; k < 2; ++k) matTemp[i][j] += matPow[i][k] * matPow[k][j];
		for (int i = 0; i < 2; ++i) for (int j = 0; j < 2; ++j) matPow[i][j] = (matTemp[i][j]) % 1000000000;
		lN /= 2;
	}
	return (matFibo[1][0] + matFibo[1][1]) % 1000000000;
}
...
int main()
{
	...
	long long lA, lB;
	cin >> lA >> lB;
	cout << ((CalcFibo(lB + 2) - CalcFibo(lA + 1) + 1000000000) % 1000000000) << "\n";
